...
export interface VarDef {
    location: Reg.Register | number; // Register or stack offset
...
export class DefUseChains {
     ...
     */
    getLocationKey(location: Reg.Register | number): string {
     ...
     */
    addDef(location: Reg.Register | number, instruction: i.Instruction, block: BasicBlock): void {
     ...
     */
    addUse(location: Reg.Register | number, instruction: i.Instruction, block: BasicBlock): void {
        const key = this.getLocationKey(location);
     ...
     */
    getDefsReachingUse(block: BasicBlock, location: Reg.Register | number): VarDef[] {
        const key = this.getLocationKey(location);
        const defs = this.m_definitions.get(key);
        if (!defs) return [];
